...
- **Nova aba `ai-jira`, que só aparece se o pacote existir na máquina.** O [ai-jira](https://github.com/Mikael-Cavalcanti/ai-jira) é um conjunto de scripts PowerShell instalado por máquina, fora do projeto Unity — não dá para declarar como dependência do package. Então a janela procura por ele (variável `JIRA_CLI_HOME`, senão `~/.ai-jira`, confirmando com `bin/jira-lib.ps1`) e revela a aba só quando encontra. Em macOS e Linux a aba nunca aparece, que é o resultado certo: os scripts são PowerShell e não há o que rodar lá.
- **Um botão por comando** — `jira-init`, `jira-card`, `jira-pr`, `jira-sync` — cada um abrindo uma conversa nova na aba Agente. A aba não executa script nenhum: as skills do ai-jira foram escritas para serem conduzidas por um agente, e um segundo caminho de execução seria um segundo conjunto de bugs sem capacidade nova.
