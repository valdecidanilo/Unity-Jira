...
- **Arquivo `.env` do agente, criado na importação.** Ao carregar o package, um `.env` é criado na raiz do projeto já com `JIRA_URL`, `JIRA_EMAIL` e `JIRA_API_TOKEN` (vazios) e as opções de CLI comentadas — é o que permite ao agente consultar o Jira sozinho durante o chat. Em `Configurações → Variáveis do agente` dá para ler, editar e salvar, com **Preencher com a conexão** para reaproveitar a conexão da aba Conexão em vez de digitar o token de novo. As variáveis são exportadas no script lançador antes de chamar a CLI, porque a execução é um processo destacado e um environment montado no lado do Editor não chegaria nele. Variável vazia não é exportada. Como o arquivo guarda um token, ele é **adicionado ao `.gitignore`** automaticamente e o cabeçalho avisa para não commitar.
