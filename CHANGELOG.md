...
- **A execução deixa de parar em "caminho fora dos diretórios permitidos".** A CLI confina o run ao diretório de trabalho, e tudo que o fluxo do Jira precisa mora fora dele: os scripts do ai-jira, as skills e o `config.json` com os projetos, campos e status reais da instância. O agente lia isso, era recusado no meio da tarefa — depois de já ter dito que ia atualizar o card — e não havia como responder o pedido de permissão numa execução headless. Agora cada run recebe `--add-dir` para a instalação do ai-jira, a pasta do `config.json` quando está em outro lugar, `~/.claude/skills` (ou `~/.codex/skills`) e a raiz do projeto Unity quando ela fica fora do repositório configurado.
- **Só diretório que existe é liberado, e nada mais largo que isso.** Um caminho inexistente faz a CLI recusar a flag, e a execução falharia por um motivo sem relação com a tarefa; o que já está dentro do diretório de trabalho não é repetido. Do home da CLI entra apenas a subpasta `skills` — ali também moram as configurações do desenvolvedor e todas as transcrições que ela já escreveu, e nada disso é assunto deste package. A pasta pessoal inteira, nunca.
- **O grant fica na auditoria.** O `request.json` da execução passa a registrar `additionalDirectories`, então o que aquele run podia alcançar é respondível depois só com o diretório dele, em `Library/JiraAgent/<runId>`.
- **`Configurações → ai-jira` mostra a linha *Acesso do agente*** com os diretórios liberados. Era a falha impossível de diagnosticar de dentro da transcrição: o agente dizia que o caminho estava fora dos diretórios permitidos e nada na janela dizia quais eram.
- **Leitura e `git` de leitura pré-aprovados.** `Read`, `Glob`, `Grep`, e `git show`, `shortlog`, `blame`, `ls-files`, `describe`, `remote -v`/`get-url` entram na lista. É o que "veja o que já foi feito e atualize o Jira" usa do começo ao fim, e uma execução headless não tem quem responda o pedido de permissão — o que não está liberado de antemão é negado no primeiro passo. Liberar ferramenta e liberar caminho são coisas separadas, e as duas precisavam estar no lugar.
- **O terminal interativo aberto pela janela recebe o mesmo grant**, porque ele existe para continuar a conversa que está na tela — e uma sessão que não vê a instalação do ai-jira responde diferente daquela que ela continua.
- **Comando do ai-jira em "Somente leitura" deixa de ser inútil.** Os quatro comandos existem para mudar alguma coisa — criar o card, mover, abrir o PR —, então nessa postura eles não tinham como funcionar. Agora aquela execução, e só ela, sobe para *Editar sem perguntar*, e a janela avisa na linha de status: quem escolheu somente leitura escolheu, e descobrir por transcrição que foi sobrescrito é pior do que ser avisado na hora.
- A tabela de permissões do README dizia que *Somente leitura* era o padrão; o padrão é *Editar sem perguntar* desde a 0.10. Corrigido.
- Em **Codex** nada muda: `--add-dir` não existe lá, o sandbox dele já lê fora do diretório de trabalho, e a escrita continua restrita ao repositório.

## 0.11.0 - 2026-08-31

- **Integração com o [ai-jira](https://github.com/Mikael-Cavalcanti/ai-jira), conduzida pelo chat.** Os comandos do ai-jira passam a ser digitados no chat da aba Agente com barra — `/jira-card`, `/jira-sync`, `/jira-pr`, `/jira-init` — e o prefixo `jira-` é opcional, então `/card` também vale. O que você escrever depois do comando vai junto como contexto. A barra não é a sintaxe da CLI: uma execução headless não tem prompt interativo para recebê-la, então o que ela dispara é uma reescrita local para a mesma instrução que a skill espera. Um comando que não existe é recusado com a lista dos que existem, em vez de virar a primeira linha de um prompt e custar um turno. A prontidão é **por comando**, não global: numa instalação nova só o `/jira-init` roda — ele é quem produz o `config.json` e imprime o setup das credenciais, então exigir os dois dele seria travar justamente o comando que os resolve. Os outros dizem qual peça falta e qual comando a fornece.
- **`Configurações → ai-jira` instala e diagnostica.** O ai-jira é um conjunto de scripts PowerShell instalado por máquina, fora do projeto Unity — não dá para declarar como dependência do package. A seção detecta (variável `JIRA_CLI_HOME`, senão `~/.ai-jira`, confirmando com `bin/jira-lib.ps1`) e mostra uma checklist de cinco linhas, ordenada por dependência: instalação, PowerShell, credenciais, `config.json` e GitHub CLI. Cada uma falta de um jeito diferente e cada uma tem a sua correção ao lado. O `gh` aparece como opcional, porque só o `/jira-pr` depende dele.
- **Botão de instalar, em dois cliques.** O primeiro mostra a linha de comando exata — `git clone` (ou `git -C … pull --ff-only`) e o `install.ps1` —, o segundo executa e mostra a saída dos dois passos separadamente, para uma falha dizer qual metade quebrou. Rodar script de outro repositório na máquina de alguém é coisa que a pessoa tem que poder ler antes, e reproduzir à mão depois. A URL do clone é constante no código, nunca montada a partir de algo lido do disco ou vindo de transcrição de agente.
- **O agente passa a preferir as skills do ai-jira.** As instruções do projeto ganham uma seção dizendo que `jira-card`, `jira-pr` e `jira-sync` têm precedência sobre o `jira.sh` e sobre `curl` cru — elas leem os projetos, campos e status reais da instância do `config.json` do próprio ai-jira. O `jira.sh` continua para o que nenhuma skill cobre: ler uma issue citada no prompt, buscar por título, comentar. Sem isso o agente ficava com dois manuais de Jira no contexto e escolhia um por turno.
- **`JIRA_BASE_URL` e `JIRA_CLI_HOME` exportados para a execução.** O ai-jira lê o host em `JIRA_BASE_URL` e este package sempre gravou `JIRA_URL` — mesmo valor, nome diferente, e sem o alias todo comando começava pedindo uma URL que já estava preenchida na aba Conexão. Nada é sobrescrito: quem definiu a variável à mão quis aquilo.
- **Comandos pré-aprovados no modo headless**, mas só em máquina que tem a instalação: as quatro skills, os dois hosts de PowerShell e `gh pr`/`gh repo view`/`gh auth status` quando o GitHub CLI responde. Uma execução headless não tem quem responda pedido de permissão, então o que não está aqui é negado no meio da tarefa — depois de o agente já ter dito que estava criando o card.
- **A aba diz o que está faltando antes de você esbarrar.** PowerShell fora do PATH, `gh` ausente (só o `jira-pr` depende dele) e skills não registradas para a CLI configurada são avisos, não erros que escondem os comandos que ainda funcionam. Cada um falha de um jeito que parece agente desobedecendo em vez de dependência ausente. Botão de copiar diagnóstico com a trilha completa de busca.
- **A checklist não lê o token.** Das credenciais só se testa presença, e só presença entra na trilha de diagnóstico — o botão *Copiar diagnóstico* existe para o texto ser colado em outro lugar, e a única garantia sólida de que um `JIRA_API_TOKEN` não vai junto é o valor nunca ser lido.
- **Indicador de "Executando..." animado no chat do agente.** Um turno passa períodos longos sem produzir nada na transcrição — lendo arquivos, raciocinando — e um chat completamente parado é indistinguível de um que morreu. Agora há uma linha entre a transcrição e o campo de mensagem com o spinner do Editor, reticências pulsando e o último passo reportado (`Executando...  ·  Read Player.cs`). Fica fora da área de rolagem, senão a própria saída que ele reporta o empurraria para fora da tela.
